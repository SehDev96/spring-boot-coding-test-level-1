...
public class RevenueController {
    ...
    @GetMapping(path = "/{id}")
    public ResponseEntity<?> getRevenueById(@PathVariable Integer id){
        // todo exceptions
        return new ResponseEntity<>(revenueService.findById(id), HttpStatus.OK);
    }

    @PostMapping(path = "/add")
    public ResponseEntity<?> insertNewRevenue(@RequestBody Revenue revenue){
        return new ResponseEntity<>(revenueService.saveRevenue(revenue),HttpStatus.CREATED);
    }

    @DeleteMapping(path = "/remove/{id}")
    public ResponseEntity<?> deleteRevenueById(@PathVariable Integer id){
        revenueService.deleteRevenue(id);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @PostMapping(path = "/update")
    public ResponseEntity<?> updateRevenue(@RequestBody Revenue revenue){
